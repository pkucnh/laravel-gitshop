		...
		<div class="col-md-12" >
			...
			<div class="row" style="background: cadetblue;padding: 15px">
				...
				<div class="col-md-6 logo"  style="color: #fff">
					<p>Chào bạn <strong style="color: #000;text-decoration: underline;">{{$cart_array['product_name']}}</strong></p>
				</div>
				...
				<div class="col-md-12">
					...
					<p>Mã đơn hàng : <strong style="text-transform: uppercase;color:#fff">{{ $ordercode_mail['order_code'] }}</strong></p>
					<p>Mã khuyến mãi áp dụng : <strong style="text-transform: uppercase;color:#fff">{{$code['coupon_code']}}</strong></p>
					<p>Phí ship hàng : <strong style="text-transform: uppercase;color:#fff">{{ $fee}}</strong></p>
					...
					<p>Địa chỉ nhận hàng : 
						@if($shipping_array['shipping_adress']=='')
							<span style="color:#fff">không có</span>
						@else
							<span style="color:#fff">{{$shipping_array['shipping_adress']}}</span>
						@endif
					</p>	
					<p>Số điện thoại : 
					...
					<table class="table table-striped" style="border:1px">
						...
						<tbody>
							...
							@php 
							$sub_total =   $cart_array['product_amount']*  $cart_array['product_price'];
							$total+=$sub_total;
							@endphp	

							<tr>
								<td>{{  $cart_array['product_name']}}</td>
								<td>{{number_format(  $cart_array['product_price'],0,',','.')}}vnđ</td>
								<td>{{  $cart_array['product_amount']}}</td>
